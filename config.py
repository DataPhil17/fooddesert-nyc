...
print("\n📡 Fetching ACS 2023 data from Census Bureau...")
...
census_df        = acs_raw_df.merge(crosswalk_df, on="zip_code", how="left")
census_df["year"] = int(ACS_YEAR)

print(f"\n📊 ACS Data Summary:")
print(f"   ZIP codes with data:       {census_df['zip_code'].nunique()}")
print(f"   ZIP codes missing income:  {census_df['median_income'].isna().sum()}")
print(f"   Avg median income:         ${census_df['median_income'].mean():,.0f}")
print(f"   Avg poverty rate:          {census_df['poverty_rate'].mean():.1f}%")
print(f"   Total population covered:  {census_df['population'].sum():,.0f}")
...
inspections_df = pd.DataFrame(inspections_raw)
inspections_df.rename(columns={"zipcode": "zip_code"}, inplace=True)
inspections_df["zip_code"] = (
    inspections_df["zip_code"].astype(str).str.strip().str[:5]
)
inspections_df = inspections_df[
    inspections_df["zip_code"].isin(NYC_ZIPS)
].copy()

inspections_df["grade_date"] = pd.to_datetime(
    inspections_df["grade_date"], errors="coerce"
)
inspections_df = (
    inspections_df
...
inspections_df = inspections_df.merge(crosswalk_df, on="zip_code", how="left")
inspections_df["is_grade_A"] = (inspections_df["grade"].str.upper() == "A")
inspections_df["grade_date"] = inspections_df["grade_date"].apply(
...
farmers_df = pd.DataFrame(farmers_raw) if farmers_raw else pd.DataFrame()
...
if not farmers_df.empty:
    ...
    # No ZIP column — assign neighborhood via lat/lon proximity
    if "latitude" in farmers_df.columns and "longitude" in farmers_df.columns:
        print("   ℹ️  No ZIP in farmers data — assigning neighborhood via lat/lon proximity")

        farmers_df["latitude"]  = pd.to_numeric(farmers_df["latitude"],  errors="coerce")
        farmers_df["longitude"] = pd.to_numeric(farmers_df["longitude"], errors="coerce")
        ...
        farmers_df["zip_code"] = farmers_df.apply(
            lambda r: find_nearest_zip(r["latitude"], r["longitude"]), axis=1
        )
        farmers_df = farmers_df[farmers_df["zip_code"].isin(NYC_ZIPS)].copy()
        farmers_df = farmers_df.merge(crosswalk_df, on="zip_code", how="left")
        matched = farmers_df["neighborhood"].notna().sum()
        print(f"   ✓ Matched {matched} of {len(farmers_df)} markets to neighborhoods via lat/lon")
...
upsert_collection(db, "census",      census_df)
upsert_collection(db, "inspections", inspections_df)
upsert_collection(db, "crosswalk",   crosswalk_df)

if not farmers_df.empty:
    upsert_collection(db, "farmers", farmers_df)
else:
    print("   ⚠️  farmers: skipped (empty DataFrame)")

print(f"\n🗄️  MongoDB collections:")
for col_name in ["census", "inspections", "farmers", "crosswalk"]:
    try:
...
datasets = {
    "Census (ACS 2023)": census_df,
    "Inspections":       inspections_df,
}
if not farmers_df.empty:
    datasets["Farmers Markets"] = farmers_df
...
print(f"\n✅ config.py complete — next: collect_snap.py")